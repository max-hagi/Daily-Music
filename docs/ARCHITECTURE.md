...
**Badges (gamification).** A habit-reward layer, fully *derived* from data
already synced — no backend table (spec Approach C: a `BadgeService` protocol
seam reserves a future Supabase/friend-profile source). The engine lives in
`BadgeCenter`, an app-level `@Observable` held by `AppEnvironment` (not the
Insights tab), so the earn celebration can fire over *any* screen. Data flow:
the live stores → a `BadgeInputs` snapshot → the pure `BadgeDeriver` →
`[EarnedBadge]` → `BadgeCenter` (which builds the summary, orders `recent`, and
diffs the celebration queue) → a **recently-earned shelf** on Insights that taps
into the full `BadgesView` page (hero = earned count + current streak, an "In
Progress" tiered grid, and an earned-only "Moments" grid). Six **tiered** badges
(streak, mint, crate, saves, ratings, rescues — each with a threshold ladder +
progress) and six one-time **moment** badges (first mint, perfect week, comeback,
night owl, flawless month, archetype revealed — **hidden entirely until earned**,
a pleasant surprise). `BadgeDeriver` reuses `Streak.compute` and `ListenStatus.of`
rather than re-deriving state, so it stays pure and fixture-tested
([BadgeTests](Daily%20MusicTests/BadgeTests.swift)). `MainTabView` hosts the earn
celebration: a `.task(id:)` over the listens/favorites/ratings signal (plus a
scene-phase refresh) re-runs `BadgeCenter.refresh()` after badge-earning actions,
and an overlay presents the [BadgeCelebrationCard](Daily%20Music/Views/Components/BadgeCelebrationCard.swift)
(one card at a time) over the whole tab bar. `BadgeSeenStore` (UserDefaults)
records already-celebrated badge+tier keys (baselines silently on first run so
existing users aren't spammed); `BadgeEarnLog` records first-earned timestamps to
order the "recently earned" shelf. Neither affects whether a badge is *earned* —
only celebration and ordering. See [Badge](Daily%20Music/Models/Badge.swift) · [BadgeInputs](Daily%20Music/Models/BadgeInputs.swift) · [BadgeDeriver](Daily%20Music/Services/BadgeDeriver.swift) · [BadgeService](Daily%20Music/Services/BadgeService.swift) · [BadgeSeenStore](Daily%20Music/Services/BadgeSeenStore.swift) · [BadgeEarnLog](Daily%20Music/Services/BadgeEarnLog.swift) · [BadgeCenter](Daily%20Music/ViewModels/BadgeCenter.swift) · [BadgesView](Daily%20Music/Views/BadgesView.swift).
